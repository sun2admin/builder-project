...
7. ✅ DONE (verified 2026-05-07 via smoke tests) — Tool subcommands implemented:
   ...
   - **Caveats discovered in smoke tests** (status as of 2026-05-07):
     ...
     - **F3 (output sparsity for builder-project run) ✅ FIXED (commit `e7003f3`):** `analyzers/dockerfile.py` replaced the hardcoded `.devcontainer/devcontainer.json` lookup with `_find_devcontainer_path()` priority probe: (1) `.devcontainer/devcontainer.json` (spec canonical), (2) `.devcontainer.json` (root alt), (3) `.devcontainer/<name>/devcontainer.json` (spec named configs), (4) `<name>-devcontainer/devcontainer.json` (project convention — e.g. builder-project's `layer4-devcontainer/`). New `tests/test_dockerfile_paths.py` pins priority order with 11 unit tests. Verified 2026-05-07: fresh `--out-dir` re-run on `sun2admin/builder-project` populates `container.*` (5 volumes, 5 env vars, post_start chain, 5 extensions, vscode_settings, workspace_mount/folder, wait_for, shutdown_action). The prior footnote about `anthropics/claude-code` "also empty" was stale — that cache has been correctly populated all along since claude-code uses the canonical path. Audit of all 10 cached repos: 8 are correctly empty (no devcontainer.json at any of the 4 known patterns upstream — verified via `gh api repos/.../contents`), 2 are correctly populated. Builder-project cache refreshed at the same commit as this plan update.
...
12. ✅ DONE (verified 2026-05-08) — End-to-end scaffold of `builder-project` via `/build-stack` produces a structurally complete devcontainer.json. Layer 12a (skill flow) verified via `--dry-run` with scripted answers (project repo `sun2admin/builder-project`, defaults for category + project name, `claude` primary, `use_recommended_l3=true`, no extra plugins) → emits well-formed schema-v3 build.json. Layer 12b (tool composition) verified by feeding that build.json to `python -m build_stack validate` (rc=0) then `compose` (rc=0); resulting `devcontainer.json` was `jq -S` diffed against the hand-written reference at `layer4-devcontainer/devcontainer.json`. **Match**: runArgs (NET_ADMIN/NET_RAW), forwardPorts ([8888]), remoteUser (claude), all 5 mounts (2 named volumes + data + 2 cred files), workspaceMount/Folder, the 5-step postStartCommand chain (byte-identical), waitFor, shutdownAction, all 5 vscode extensions (alpha-sorted by emit.py), all 5 vscode settings (incl. `terminal.integrated.profiles.linux.claude-bash` with `--rcfile claude-rc.sh`). **Differences by design**: `name` auto-derived ("Claude Build Container" → "builder-project (build-stack)"); `image` uses canonical `claude-plugins-recommended:latest` per the 2026-05-06 architecture pivot to a single recommended L3 (the reference's `claude-plugins-a7f3d2e8:latest` predates that pivot); extension array sort order. **F8 (deferred follow-up):** `compose._compose_credentials` (compose.py:143-169) defaults all detected credentials to `containerEnv` passthrough, producing `GITHUB_TOKEN: ${localEnv:GITHUB_TOKEN}` and `SSH_AUTH_SOCK: ${localEnv:SSH_AUTH_SOCK}` in the generated devcontainer.json. The reference project, however, has *both* file-mount delivery (`/run/credentials/gh_pat`, `gh_claude_ed25519`) and init scripts (`init-gh-token.sh`, `init-ssh.sh`) that consume those files — so the compose-tool defaults layer **on top** of an existing delivery mechanism, redundantly for `GITHUB_TOKEN` and overridingly for `SSH_AUTH_SOCK` (host agent forwarded → replaces the internal-agent socket `/home/claude/.ssh/agent.sock` set up by init-ssh.sh). The compose layer already has an `overrides.credentials_delivery` mechanism (compose.py:145, supports `"mount"` to skip env-passthrough); it just isn't surfaced through the `/build-stack` skill yet. Resolution path is design-judgment (auto-detect mount-already-present vs. expose skill-level override flag vs. document-and-keep-default) and is deferred to a follow-up.
