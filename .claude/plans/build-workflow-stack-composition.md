...
- **`myclaude` alias placement (added 2026-05-07):** Currently lives at L4 — `layer4-devcontainer/scripts/claude-rc.sh` referenced via tasks.json + `terminal.integrated.profiles.linux.claude-bash` default profile (see `layer4-terminal-launch-fix.md`). Long-term, the alias is an "AI CLI launch ergonomic" concern, not a devcontainer concern — belongs in L2 image bake (`/etc/bash.bashrc.d/claude-aliases.sh` or similar) so every stack inherits without per-L4 template config. Defer until L2 features-vs-image-bake question (above) resolves — the two should land together.
- **`myclaude` ↔ `load-projects.sh` coupling (added 2026-05-07):** Alias depends on `~/live-project` file written by `load-projects.sh -live <repo>`. Sandbox builds (no `project_repo`) skip the `-live` flag → `~/live-project` absent → alias falls back to `cd ~` (no project context). Two open paths: (a) require a default "template project" repo for sandbox builds so alias has somewhere to land; (b) make alias smarter — fall back to `/workspace` (workspace root) when `~/live-project` missing instead of `~`. Decide before sandbox-mode build path is wired in `/build-stack`.
